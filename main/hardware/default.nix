# Bloom Nix branding configuration - Traditional NixOS module
{ config, lib, pkgs, ... }:
...
in {
  ...
  # Set the system name (if this option exists in traditional NixOS)
  ...
  # SDDM theme customization (when SDDM is enabled)
  services.displayManager.sddm.settings = lib.mkIf config.services.displayManager.sddm.enable {
    Theme = {
      Background = lib.mkForce "${brandingAssets}/sddm-background.png";
    };
  };
